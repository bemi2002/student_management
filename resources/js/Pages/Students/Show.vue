<template>
  <AuthenticatedLayout>
    ...
    <div class="container mx-auto px-4 py-6 max-w-7xl">
      ...
      <!-- Header Actions -->
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 p-5 bg-gradient-to-r from-indigo-50 to-white rounded-xl shadow-sm border border-gray-100">
        <div class="mb-4 sm:mb-0">
          <h1 class="text-3xl font-bold text-gray-900">{{ student.full_name }}</h1>
          <div class="flex items-center mt-2 text-gray-600">
            <svg class="h-5 w-5 mr-1.5 text-indigo-500" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <Link v-if="canEdit" :href="route('students.edit', student.id)"
            class="flex items-center bg-gradient-to-r from-blue-500 to-blue-600 text-white px-4 py-2.5 rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all duration-200 shadow hover:shadow-md">
            <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
          ...
          <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <div class="px-6 py-5 bg-gradient-to-r from-emerald-500 to-emerald-600">
              <h2 class="text-xl font-bold text-white flex items-center">
                <svg class="h-6 w-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div v-if="student.dropout_history?.length">
                <div class="grid grid-cols-1 gap-4">
                  <div v-for="record in student.dropout_history" :key="record.id" 
                       class="p-5 border border-red-50 bg-red-50 rounded-xl hover:bg-red-100 transition-all duration-200">
        ...
        <div class="space-y-8">
          ...
          <div v-if="canAssign" class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden sticky top-6">
            <div class="px-6 py-5 bg-gradient-to-r from-violet-500 to-violet-600">
              <h2 class="text-xl font-bold text-white flex items-center">
                <svg class="h-6 w-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div v-if="availableEnrollments?.length">
                <div class="mb-6">
                  ...
                  <div class="relative">
                    ...
                    <select v-model="selectedEnrollment" 
                            class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-violet-500 outline-none transition-all appearance-none bg-white">
                ...
                <div class="space-y-3">
                  <button @click="assignEnrollment"
                    ...
                    :class="selectedEnrollment ? 'bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700' : 'bg-gray-300 cursor-not-allowed'"
...
function getStatusBadgeClass(status) {
  ...
  const statusClasses = {
    'completed': 'bg-emerald-100 text-emerald-800 px-3 py-1.5 rounded-lg text-sm font-bold',
    'in progress': 'bg-blue-100 text-blue-800 px-3 py-1.5 rounded-lg text-sm font-bold',
    'pending': 'bg-amber-100 text-amber-800 px-3 py-1.5 rounded-lg text-sm font-bold',
    ...
    'active': 'bg-emerald-100 text-emerald-800 px-3 py-1.5 rounded-lg text-sm font-bold'
  };
